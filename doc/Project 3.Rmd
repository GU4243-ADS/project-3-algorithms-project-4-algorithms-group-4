...
EM_train <= function(data, k, C = 5, tau = 0.01, ITER = 100){ 
  ...
  #### Step 2 - Expectation
    ...
    aic_numer <- mu * pre_aic # gives the numerator of aic. dims = (C, i) #I am assuming C clusters are still oriented 1:C in the same order in both mu and pre_aic
    
    aic_denom <- colSums(aic_numer)  # every user gets the same denom over all the clusters (here, 1 cluster = 1 col). dims = vector length(users)
    
    aic <- t(aic_numer)/aic_denom # dims = (users, clusters)
