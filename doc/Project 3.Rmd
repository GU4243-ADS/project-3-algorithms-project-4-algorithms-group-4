...
EM_func <= function(data, C = 1, tau = 0.05, iter = 100){ 
  # function arguments include:
      #data: user-item matrix
      #C: number of clusters
      #tau: threshold for termination when values converge, using tau = 0.05 from previous year's code but no particular understanding
 ...
 # function output = ???
  ...
  gamma <- array(0, dim = c(length(users), k, C))
  
  cluster.probs <- matrix(0, nrow = length(users), ncol = C) # cluster assignment matrix
  ...
  # Mu 
  mu <- apply(cluster.probs, 2, sum)/length(users)
  
  # Gamma
  
  users, k, c
  
  gamma <-  subset of users who rated movie j and rated it k (id those users on the c=1 col of cluster.probs and sum)
  
 
gamma <- function(k=1) {
  k = 1
  C = 6
  cluster.probs <- matrix(1, nrow = length(users), ncol = C) # dims = users, clusters
