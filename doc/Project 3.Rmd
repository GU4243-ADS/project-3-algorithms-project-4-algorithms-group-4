...
EM_train <= function(data, k, C = 5, tau = 0.01, ITER = 100){ 
  ...
  mu <- rep(1/C, C) 
  gamma <- array(1/k, dim = c(items, C, k)) # dims = items(movies), clusters, ratings
  aic <- matrix(NA, nrow = users, ncol = C) # cluster assignment/probability matrix, dims = users, clusters
  ...
  pre_aic[i,,] <- temp[i,,] # extract only the row for which the log sums over the correct subset of gammas_(k=i, j, c) 
    }
    ...
    #basically took the logsum/product of all the movies rated 1 (then 2,...) for user i in cluster 1 wieghts, then log summed all those log sums over all ratings k for each user i
    
    
     new = array with dims = 4,6,2
    t(colSums(new)) = matrix with rows = 3rd margin slices in order, cols = colsums i.e. dims = 
    
      dims = (movies/items, users, ratings)
    (items, clusters)
    
    (movies, clusters, ratings)
...
#rowSums(aperm(new, c(1, 3, 2))[,,1]) # this sums over k's for each movie. The sums should equal 1. They do!!!  but I have some NaNs which are introduced when I divide the numerator by the denominator. ... what to do... logs...? just set them to 0?? 
  ...
  #### Convergence Conditions using l-2 norm 
  ...
  ### Hard Assignment
  hard.assignments <- (aic == rowMins(aic)) + 0
...
predict <-function(UI_matrix, j, aic, gamma){
  n_users <- nrow(UI_matrix)
  
  predict_rank <- matrix(0, nrow = n_users)
  for (j in 1:n_items){ 
    for (k in 1:6){
      predict_rank <- predict_rank + k * aic %*% gamma[, j]
    }
  }
  return(predict_rank)
}

# not sure if this function will actually work, but I think the math is pretty close to what we need. Feel free to change it in any way; this was just my first attempt. 


bayesian_predict <- function(hard.assignments, active.users) {
