...
bayesian_predict <-function(data, range, hard.assignments, gamma){
  n.users <- dim(data)[1]


  predict.rank <- matrix(0, nrow = n.users)
    for (k in 1:6){
      predict.rank <- predict.rank + k * hard.assignments %*% gamma[j,  ,k] 
    }
  }
...
### Test error
```{r}
what.to.predict <- which(!is.na(movie_UI_test))
...
```{r}
mov.train <- movie_UI # to clarify that this is training data, Note: the unrated user-movie combos are NA's
data <- movie_UI # subset for speed (until finished)
...
EM_train <- function(data, k, C = 5, tau = 0.01, ITER = 100){ 
  ...
  #users <- rownames(data)
  #items <- colnames(data) # used items as a generic for both movies and vroots
  mu <- matrix(1/C, nrow = C) # vector length C
  gamma <- array(1/6, dim = c(k, n.items, C)) # dims = ratings, items(movies), clusters
    ...
    for(i in 1:n.users){
      ...
      for(c in 1:C) {      
        for(x in 1:length(items)){ 
          aic.numer[i, c] <- mu[c]*gamma[rank[[x]], items[[x]], c] # calculate the numerator of aic, with the formula above. 
...
movie.predictions <- EM_train(mov.train, k = 6, C = 12)
```
