...
EM_func <= function(data, C = 1, tau = 0.01, ITER = 100){ 
 ...
 # function output = ???
  ...
  init <- sample(1:users, C)

  
  #### Step 2 - Expectation 
  while(is.conv > tau | iter < ITER){
    L <- log(1/6) # means of rankings -- 1/6? 
    gamma <- exp(data %*% t(L)) * mu
    cluster.probs <- matrix(rep(1/rowSums(gamma), C), length(users), C) * gamma
...
#system.time(gamma())
  ...
  is.conv <- norm(mu_hat - mu_pre, type = "O") # measure of the change in cluster assignments
...
hard.cluster <- apply(cluster.probs, 1, which.max) # hard cluster assignments
```
