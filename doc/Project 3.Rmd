...
=======
    ...
    for (i in 1:k) {
      log(data + gamma )
      gamma.sum <- apply(gamma, MARGIN = c(items), sum)
    }
    
>>>>>>> 67f0c93f3ce2b1ce9dc2c1c83532fe8e1d577a1d
...
bayesian_predict <-function(data, j,  aic, gamma){
  n_users <- dim(data)[1]

  
  predict_rank <- matrix(0, nrow = n_users)
    for (k in 1:6){
      predict_rank <- predict_rank + k * aic %*% gamma[, j, k] # non-conformable arguments... 
    }
  }
