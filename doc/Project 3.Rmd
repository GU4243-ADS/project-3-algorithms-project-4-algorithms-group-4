...
EM_train <- function(data, k, C = 5, tau = 0.01, ITER = 100){ 
  ...
  ######################### Step 2 - Expectation ####################
    ...
    aic_denom <- rowSums(aic_numer)  # every user gets the same denom over all the clusters (here, 1 cluster = 1 col). dims = vector length(users)
    
    #aic <- aic_numer/aic_denom
    aic <- exp(log(aic_numer) - log(aic_denom)) # dims = (users, clusters) # NaNs introduced.... :(
